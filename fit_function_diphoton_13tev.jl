...
data_myy = vec(readdlm(joinpath(data_dir, "data_myy.dat"), comments=true))
mc = readdlm(joinpath(data_dir, "mc_signal_myy_weights.dat"), comments=true)
mc_myy = mc[:, 1]
mc_w   = mc[:, 2]
...
println("\n" * "="^50)
println("  SIGNAL YIELD (N_sig) SUMMARY")
println("="^50)
println("Best-fit N_sig (refined mode):    ", round(fit_par_values.N_sig, digits=2))
println("Posterior mean N_sig:             ", round(N_sig_mean, digits=2))
println("Posterior median N_sig:           ", round(N_sig_median, digits=2))
println("Posterior std dev:                ", round(N_sig_std, digits=2))
println("68% credible interval:      [", round(N_sig_lower_68, digits=2), ", ", round(N_sig_upper_68, digits=2), "]")
println("95% credible interval:      [", round(N_sig_lower_95, digits=2), ", ", round(N_sig_upper_95, digits=2), "]")
println("="^50 * "\n")
...
vspan!([N_sig_lower_95, N_sig_upper_95], alpha=0.1, color=:red, label="95% CI")
