...
#Data cut - extracting only real data
data_mask = (df.dataset .== "data")
df = df[data_mask, :]
cutflow["Data"] = nrow(df)
...
histogram(
    ...
    label="Diphoton Mass",
    title="Diphoton Invariant Mass - Only Data",
    color=:pink
)
...
CSV.write(joinpath(homedir(), "separated_and_cut_data_13TeV.csv"), df)