...
using CSV, DataFrames, StatsBase, BAT, Distributions, Measures, Plots, DensityInterface, Optim
...
p1 = plot(
    ...
    markersize = 5,
    color = :black,
    bottom_margin = -2mm, # Reduce gap between plots
    xaxis = false         # Hide x-axis labels/ticks on the top plot
)

# Add background
bar!(p1, fit_bin_centers, collect(fit_background), 
     bar_width = fit_bin_width * 0.9,
     label = "Background (MC)", color = :turquoise4, alpha = 0.5)

# Add Signal
bar!(p1, fit_bin_centers, best_fit.mu .* collect(fit_signal), 
     bar_width = fit_bin_width * 0.9,
     bottom = collect(fit_background),
     label = "Signal * μ=$(round(best_fit.mu, digits=2))", color = :red, alpha = 0.7)

# Add Fit Line
plot!(p1, fit_bin_centers, expected_bestfit, 
      label = "Total fit (μ*S + B)", lw = 2, color = :black)

# --- 2. Residual Plot (Bottom) ---
expected_total = (best_fit.mu .* fit_signal) .+ fit_background
data_minus_fit = fit_data_observed .- expected_total
...
p2 = plot(
    ...
    grid = true,
    gridalpha = 0.15,
    top_margin = -2mm # Reduce gap between plots
)
# Add a zero line for reference
hline!(p2, [0], color = :black, linestyle = :dash, label = "")

# --- 3. Combine them ---
# Define a layout where the top plot is 70% height and bottom is 30%
l = @layout [a{0.7h}; b]

full_plot = plot(p1, p2, layout = l, link = :x, size = (900, 700))
...
p_bat = plot(
    ...
    :mu, 
    bins = 100, 
    title = "Marginalized Posterior: Signal Strength",
    xlabel = "μ",
    ylabel = "Density",
    color = :viridis # BAT defaults to a multi-color scale, 
                     # but you can specify schemes here
)
...
p_matrix = plot(
    ...
    globalmode = true,
    marginal = true,
    title = "Marginalized Posterior for Signal Strength (μ)",
    xlabel = "μ",
    legend = :topright
)
