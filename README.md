...
Of important note is the choice of the type of polynomial for $B_i$. The 2022 diphoton paper [[5](https://arxiv.org/abs/2202.00487)] suggests that background events consistent with the diphoton channel follow a distribution that counts like a polynomial of fourth degree, hence this specific choice, a choice that was not necessary in the four-lepton model. 
...
In the "fit_functions" folder, the code in the file "fit_function_4lep_13tev.jl" plots the statistical model (below) applied to the four-lepton data to give a fit-function imposed on the background and signal simulated (MC) data in the Higgs region (~ 110-140 GeV). The curve with the best fit signal-strength value ($\mu \approx 2.2$) is in black with the standard model prediction ($\mu=1$) in green (dotted).

<img width="600" height="400" alt="lepton fit function plot" src="https://github.com/user-attachments/assets/f08889a3-4c92-4ebd-bfe3-63c5af90ee22" />

For reproduction, these quantities can be obtained directly from the posterior samples generated by BAT.jl. The posterior mode provides the best-fit value, while the mean, median, standard deviation, and credible intervals can be calculated from the sampled values of $\mu$. The code we use to obtain these statistics is located in the "fit_functions" folder, in the file "fit_function_4lep_13tev.jl" which is a julia file.
...
Below we provide a plot for the statistical model applied to the diphoton channel, similar to the four-lepton graph provided above but adapted for a different channel. This dataset includes significantly higher background events and overall events compared to the four-lepton channel, a visual difference that also scales differently on plots when comparing to the other channel and uncertainties. Code for this plot is located in the "fit_functions" folder, under the file name "fit_function_4lep_13tev.jl".

<img width="457" height="400" alt="diphoton fit function plot" src="https://github.com/user-attachments/assets/d34c3581-fd69-472b-b80b-ee7ce34f9731" />

These parameters define the fitted fourth-degree background contribution used alongside the signal model in the diphoton likelihood. Unlike the four-lepton channel, where the signal is controlled by the dimensionless signal strength $\mu$, the diphoton fit uses $N_{\mathrm{sig}}$ as a signal-yield normalization because the background is simultaneously determined by the floated polynomial parameters. The covariance plot below (produced with the BAT.jl package) pictorially represents the relationships between each parameter floated in the fitting process to give insight in to how changes in one parameter will affect the other, a metric that was excluded from the four-lepton statistical analysis due to there only being one parameter of interest in that model unlike in this one which has 6 ($N_{sig}$ and 5 polynomial coefficients). The code for this plot is also in the "fit_function_4lep_13tev.jl" file.

<img width="667" height="400" alt="diphoton covariance plot" src="https://github.com/user-attachments/assets/4aec6cbc-6e64-4ed4-bd8e-d5f30d15f074" />
...
The code we use to obtain these statistics is located in the "fit_functions" folder, in the file "fit_function_diphoton_13tev.jl" which is a julia file.

# 6. Discovery Sensitivity
## 6.1 Initial Plot Visualizations

We use the same approach for our discovery sensitivity analysis for both the diphoton and four lepton channels. In the fit function files (4l_BAT_Stats.jl for 4 lepton and fit_function_diphoton_13tev for diphoton), they each produce 4 plots along with the best fit statistics generated by the built-in BAT.jl function. The first plot simply plots the newly cut data. The second plot displays the covariance graph(s) for each of the parameters that we float. For 4l_BAT_Stats.jl, the second plot only has one covariance graph because we only floated the signal strength parameter $\mu$. The covariance graphs for fit_function_diphoton_13tev are a bit more complicated as they contain five parameters as we used a third-degree Bernstein polynomial for the background that had four coefficients along with the number of signal events N_sig. The third plot includes residuals and a narrower invariant mass window to better visualize the signal peak. The fourth plot is the same as the first covariance plot in addition to  the appropriate colors to visualize the standard deviation confidence intervals. 

## 6.2 Toy Modeling

After finding a fit, we have to test it through a rigorous discovery sensitivity analysis. First, we create toy models to replicate the variability in the LHC runs. In both of the toy generation files (ToyGenerationNEW.jl for 4 lepton and toy_generation_diphoton.jl for diphoton), they produce one graph that displays 10 toys to visualize this. To do this, it uses the binned fit for the signal spectrum from the previous BAT.jl module and poisson fluctuates each bin. This function is especially useful as we can also modify the signal strength at which we generate these toys.

## 6.3 Toy Fits

Once running the toy generation modules, we can export the function to loop it in our final files (ToyFitsNEW.jl and loopsNEW.jl for 4 lepton and toy_fits_diphoton.jl for diphoton). Using the toy generation function, we generate 200 toys for every 0.1 signal strength interval all the way up to 2.5. Each of these toys are fitted using the same process outlined by the first bayesian fit module. This time, however, we use an additional method known as the Savage-Dickey Density ratio to calculate the Bayes factor for each toy. A Bayes factor is calculated by finding the ratio between the probability of the null hypothesis (mu=0) and the probability of the alternative hypothesis (mu=median mu value in the posterior distribution). However, for almost all the toys, its posterior distribution implies that the probability of the null hypothesis is 0, when in reality this probability is a very small non-zero value. To overcome this computational limitation, we make use of a clever trick. For a single toy, after it has been fit, we take its binned marginal posterior and take the negative logarithm of each binned value. This is because we assume the posterior distribution takes a gaussian-like shape, and taking the negative log of it gives a nice even polynomial. Using these negative log likelihood values, we fit an even polynomial with no local extrema and extrapolate a negative log likelihood value for the null hypothesis by finding the polynomial’s y-intercept. After computing the Bayes factors for all 200 toys in each specific injected signal strength, it finds the median Bayes factor along with the 16th and 84th quartile and plots it. Using an unweighted power fit on all of these median values, we can finally extrapolate at what signal strength our Bayesian framework needs to cross the 5σ threshold for discovery. 

## 6.4 Results and Further Plans

For the 𝛾𝛾 channel, the 5σ threshold was found to be a signal strength (N_sig) of 750.31 ± 53.37, while for 4𝓁, the 5σ threshold was μ = 2.115 ± 0.156. From our discovery sensitivity results, our Bayesian framework is able to predict a 5σ confidence level discovery at reasonable signal strengths in both the 4𝓁 and diphoton decay channels. It is important to note that while our project focused on the interest parameter of signal strength, this same framework can further be used to characterize nuisance parameters. By incorporating these systematic uncertainties, we might be able to learn more about the constraints and thus improve our discovery sensitivity. 
