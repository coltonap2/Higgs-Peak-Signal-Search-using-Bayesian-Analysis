...
The primary difference from the four-lepton specific model is the construction of the signal and background distributions used to obtain $s_i$ and $b_i$. Just as in the four-lepton model, $s_i$ is Monte Carlo (MC) simulated and given by CERN. However, this is not true for the background ($b_i$) where it was for the four-lepton model. In order to rectify this lack of MC data for the background, we use a fourth-degree Bernstein polynomial for $b_i$ and denote it $B_i$ (where we establish “B” to stand for “Bernstein”) to differentiate from the lower-case b notation in the four-lepton model. A similar shift in notation we apply to the signal strength parameter $\mu$ which we denote $N_{sig}$ to differentiate from the four-lepton model that uses $\mu$. Once the outputs of this model for $\lambda_i(\mu)$ are obtained, they are fed through the same common likelihood framework as we describe in 5.1.

Of important note is the choice of the type of polynomial for $B_i$. [This paper](https://arxiv.org/abs/2202.00487) suggests that background events consistent with the diphoton channel follow a distribution that counts like a Bernstein polynomial of fourth degree, hence this specific choice, a choice that was not necessary in the four-lepton model. 

As we explain in section 5. on Bayesian inference, the signal strength parameter is floated in the common statistical model regardless of the format or source of $s_i$ and $b_i$ however, due to the use of the Bernstein polynomial, we must also float coefficients for $B_i$ in our fitting process. 
