...
After finding a fit, we have to test it through a rigorous discovery sensitivity analysis. First, we create toy models to replicate the variability in the LHC runs. To do this, it uses the binned fit for the signal spectrum from the previous BAT.jl module and poisson fluctuates each bin. 

```julia
toys = [generate_toy_experiment(fit_signal, fit_background, best_fit.mu) for _ in 1:10]
```

Once we have the poisson fluctuated bins, we produce a graph that displays 10 toys as a visualization. For these specific visualizations, we used the best fit signal strength as the inserted signal strength value. 

<img width="1116" height="732" alt="Screenshot 2026-08-29 at 11 59 42 AM" src="https://github.com/user-attachments/assets/d457dd1b-ef3f-41cb-9ad2-335845fcacdf" />

<img width="912" height="608" alt="image" src="https://github.com/user-attachments/assets/b4ce5c5f-a83d-44cc-bd7b-56fe9431d450" />

Notice that the poisson fluctuated bins for the 4l toys seem to have much more fluctuations than the diphoton toys. This is simply due to the low count statistics in the binned 4l fit of the signal. 

## 6.2 Toy Fits

Once running the toy generation modules, we can export the function to loop it in our final files (ToyFitsNEW.jl and loopsNEW.jl for 4 lepton and toy_fits_diphoton.jl for diphoton). Using the toy generation function, we generate 200 toys for every 0.1 signal strength interval all the way up to 2.5. 
...
Each of these toys are fitted using the same process outlined by the first bayesian fit module. This time, however, we use an additional method known as the Savage-Dickey Density ratio to calculate the Bayes factor for each toy. A Bayes factor is calculated by finding the ratio between the probability of the null hypothesis (mu=0) and the probability of the alternative hypothesis (mu=median mu value in the posterior distribution).

$$BF_{10} = \frac{P(D \mid H_1)}{P(D \mid H_0)}$$

However, for almost all the toys, its posterior distribution implies that the probability of the null hypothesis is 0, when in reality this probability is a very small non-zero value. To overcome this computational limitation, we make use of a clever trick. For a single toy, after it has been fit, we take its binned marginal posterior and take the negative logarithm of each binned value. This is because we assume the posterior distribution takes a gaussian-like shape, and taking the negative log of it gives a nice even polynomial. Using these negative log likelihood values, we fit an even polynomial with no local extrema and extrapolate a negative log likelihood value for the null hypothesis by finding the polynomial’s y-intercept. 

<img width="912" height="608" alt="image" src="https://github.com/user-attachments/assets/5ee878bd-6e21-4254-849f-242bd3cb0eb5" />

After computing the Bayes factors for all 200 toys in each specific injected signal strength, it finds the median Bayes factor along with the 16th and 84th quartile and plots it. Using an unweighted power fit on all of these median values, we can finally extrapolate at what signal strength our Bayesian framework needs to cross the 5σ threshold for discovery. 

<img width="972" height="608" alt="image" src="https://github.com/user-attachments/assets/78b4fabb-557a-4dbb-b6ed-3311c71576b6" />

<img width="1121" height="755" alt="Screenshot 2026-08-29 at 12 37 38 PM" src="https://github.com/user-attachments/assets/89babf09-ffb9-44ca-823f-cb10ebd97f57" />

Due to the computational heavy nature of this looping process, as it converges 4 MCMC chains for 200 toys for 25 different signal strengths, we recommend using a computer that has a significant amount of RAM and leaving the code to run for a few hours. 

## 6.3 Results and Further Plans
